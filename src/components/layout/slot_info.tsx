...
import { TileKey, TileSide, TileCorner, CandidateMatch } from "lib/tile_matching";
      ...
      <div className="mt-2 space-y-1">
        ...
        <div className="grid grid-cols-1 gap-x-2 font-mono text-[10px]">
          {keyInfo.edge_types.map((et, i) => (
            <div key={i} className={`flex justify-between ${getEdgeColor?.(i) ?? "text-slate-300"}`}>
            ...
            {candidates.map((c, i) => (
              ...
              >
                <div className="flex justify-between items-start font-mono text-[10px]">
                  <span className="font-bold truncate max-w-[120px]">{c.tile.file.split('/').pop()}</span>
                  <span className="flex-shrink-0 ml-1">{c.rotation}° {c.flip ? 'F' : ''}</span>
                </div>
